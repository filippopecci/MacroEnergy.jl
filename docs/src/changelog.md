...
- Added `write_formulation(case_or_system; path, formats)` to export a compact, human-readable mathematical formulation of the *generated* model (sets, variables, parameters, objective, and active constraints) as LaTeX (`.tex`, compilable to PDF) and/or Markdown (`.md`, KaTeX). Each constraint is shown once in generic form with the list of elements it applies to, using a compact short-symbol notation (e.g. `f_{e,t}`, `x_e`, `s_{g,t}`, `u_{e,t}`) documented in a Notation section. Backed by a dedicated `formulation(constraint, element)` API that mirrors `add_model_constraint!` dispatch, plus a `structural_formulations(element)` API that captures the structural relations added directly with `@constraint(...)` during model build (capacity accounting, retirement/retrofit bounds, unit-commitment logic, lossy bidirectional flow splitting, long-duration storage links, supply-segment and policy-budget balances). Balance constraints are reconstructed from `balance_data` so they display the actual signed flow terms (and demand/storage-level offsets) with their real sense and symbolic conversion coefficients (`a_role`), grouped across all structurally identical elements regardless of numeric coefficient values.
